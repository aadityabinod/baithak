...
function calculateAvailableTimeSlots(
  ...
  // Access the time_slots property correctly from the Nylas API response
  const busySlots: Array<{ start: Date; end: Date }> = [];
  
  if (nylasData.data[0] && 'time_slots' in nylasData.data[0]) {
    const freeBusyData = nylasData.data[0] as FreeBusyData;
    if (freeBusyData.time_slots && Array.isArray(freeBusyData.time_slots)) {
      busySlots.push(
        ...freeBusyData.time_slots.map((slot: TimeSlot) => ({
          start: fromUnixTime(slot.start_time),
          end: fromUnixTime(slot.end_time),
        }))
      );
    }
  }
